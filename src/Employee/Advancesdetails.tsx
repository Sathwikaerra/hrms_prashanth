...
const Advancesdetails: React.FC = () => {
  ...
  const advanceData = [
    {
      ...
      comments: 'Traveling for client meeting in Bangalore.',
      sanctionedAmount: 18000,
      approvalDate: '2025-08-03',
      approverName: 'Finance Manager',
      approverComments: 'Approved partial based on policy.',
      status: 'Approved',
    },
    {
      id: 2,
      ...
      comments: 'Covering emergency medical expenses.',
      sanctionedAmount: 15000,
      approvalDate: '2025-07-17',
      approverName: 'HR Head',
      approverComments: 'Fully approved for urgent needs.',
      status: 'Approved',
    },
    {
      id: 3,
      ...
      comments: 'Short-term personal need.',
      sanctionedAmount: 0,
      approvalDate: '2025-08-06',
      approverName: 'Department Head',
      approverComments: 'Rejected due to pending advance.',
      status: 'Rejected',
    },
  ];
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Requested Amount</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Approval Date</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedData.map((entry, index) => (
                <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{entry.purpose}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{entry.comments}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-green-700 font-semibold">
                    ₹{entry.sanctionedAmount.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{entry.approvalDate}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{entry.approverName}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{entry.approverComments}</td>
                  <td
